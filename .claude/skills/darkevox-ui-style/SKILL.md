...
- Panel morph: pill→card 240 ms OutQuart anchored at the nearest screen corner, content fading in behind it; card→pill mirrors at 180 ms. The paired-states rule: open animates, so close animates.
...
**HUD.** A frameless, always-on-top pill, bottom-center, 8 px above the taskbar, auto-width (labels elide past 320 px) with 16 px horizontal padding, 36 px tall. `cream-50` fill, hairline border, the app shadow. Content: a state dot plus one short label in `ink-900` 13 px. States: listening (blue-300 dot, pulsing), transcribing (blue-400, steady), polishing (honey-300), grounded (sage-300 mini-badge, 20 px tall), done (word count + one dot swell), error (clay-400, plain-words message, stays 4 s). Never takes focus, never accepts clicks.
...
**Panel (the floating dictation card).** A frameless, always-on-top, draggable card 380 px wide that morphs into a 56 px mic pill. Mouse contract on the mic in both states: click toggles a session, press-and-hold (over 280 ms) is push-to-talk, dragging more than 6 px moves the window (release settles it back on-screen if dropped off the edge); double-click on the pill expands; Esc collapses; right-click offers Settings and close-to-tray. The pill carries a 11 px state dot (HUD colors) and a live tooltip. The card, top to bottom: drag header (title + quiet settings/hide), mic + a two-line status block (13 px title + 11 px caption) that never lies — Listening with a live duration, Transcribing…, Polishing, Ready with the word count, Copied/Inserted confirmations that revert after 2 s, errors in `clay-600` that stay; the demoted HEARD field (72 px, filled) with an undo-take quiet action; the hero POLISHED field (120 px, raised); the segmented tone track (email/message/notes/verbatim); Copy / quiet Clear / primary Insert. Voice BUILDS on the draft (append with smart joining, `ui/draft.py`); an empty take can never wipe it. Ctrl+Enter inserts, Ctrl+Shift+C copies. Insert hands focus back to the window the user was working in before pasting.

**Tray.** Left-click opens/hides the panel; the menu opens with a status row (state dot + label), then actions. Menus are translucent, frameless, radius 10, with the app shadow — never the tight native one. The update action tells the truth: "Check for updates" rests, "Update available — install" when one exists.
