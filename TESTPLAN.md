...
The pytest suite (79 tests) covers every piece of pure logic and runs on any platform: config round-trips, the ring buffer, pause segmentation, the hotkey state machine, injection with fakes, prompt assembly, the polish pipeline with a fake LLM, and an offscreen end-to-end controller flow. `ruff check .` must be clean.
