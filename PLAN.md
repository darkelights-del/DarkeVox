...
2. **LLM defaults.** The spec leaned OpenRouter-first. User directive reversed it: **Ollama is the default backend** — `qwen2.5:3b` for polish (has to feel instant), `qwen2.5:7b` reserved in config for summarize (background work, quality over speed). OpenRouter free tier is a manual toggle whose model ID ships blank; the free-model list rotates, so the README points at the live list instead of trusting a name. The Claude API is not in the default build.
...
6. **Dev environment vs target.** Built and unit-tested on Linux/Python 3.11 in this session; the target is Windows/Python 3.12. Every Windows-bound dependency (pywin32, pynput runtime, sounddevice device I/O, Qt) sits behind an interface with a fake, so pytest runs green here. OS-integration behavior is enumerated in `TESTPLAN.md` for manual verification on the real machine — those acceptance items are *parked, not passed*.
...
- **Icon:** drawn at runtime with QPainter (mic glyph on a pastel roundrect) — no binary assets in the repo.
- **HUD:** frameless pill, bottom-center, fades in/out, pulses while listening, shows the polishing state and a "grounded" badge.
- **Settings:** one dialog — hotkeys, STT model, LLM backend, default tone, injection method. Tray menu carries tone and mode toggles.
