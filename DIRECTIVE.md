...
Phases 0-2 complete and field-running. Voice iteration 3 shipped: adaptive quiet-mic segmentation, long-segment cuts, fidelity-hardened prompts (banned lists constrain only model additions), sanitize hardening. Design pass shipped: original voice-wave mark, modern filled-field theme, overline labels, blue-500 primaries. Hourly in-session watchdog runs until 2026-07-18 14:00 UTC. Tag `v0.1.0-beta` pushed (owner publishes the Release from it). Voice dry sweep 1 returned lows only (all fixed: peak-rms decay, tap-guard and stt-timing tests); quiet-input boost added before transcription. Phase 3 Compose tab shipped (tray > Open composer). Open: owner re-test on Windows; one more dry sweep closes voice; next is phase 4 (ingest/store).
