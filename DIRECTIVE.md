...
Phases 0-3 complete and field-running (dictation, polish, panel, Compose tab, updater). Voice segment CLOSED code-side: sweep 1 lows fixed (peak-rms decay, tap guard, timing tests), sweep 2 fully dry (boost_quiet, decay math, requester routing all verified). Quiet-input boost, beam-5 decoding, adaptive segmentation, and fidelity-locked prompts shipped. Design system v2 shipped: voice-wave mark (transparent corners proven), filled fields, overlines, blue-500 primaries. Tag `v0.1.0-beta` pushed; owner publishes the Release from it. Autonomous window ended 2026-07-18 14:00 UTC; both watchdogs self-retire on their next firing. OPEN, in order: owner re-test of dictation accuracy and look on Windows (field reports reopen the voice segment if needed); phase 4 ingest/store; phase 5 summaries+Q&A; phase 6 bridge; phase 7 packaging + installer release.
